...
AddBook("Deep Work" , "Cal Newport" , 202 , true);
AddBook("Dopamine Nation" , "Anna Lembke" , 204 , false);
ShowBooks();
function Book(t,a,n,r)
{
    this.title = t ; 
    this.author = a ; 
    this.nbPages = n ; 
...
function AddBook( title , author , nbPages , Read)
{
    myLibrary.push(new Book(title,author,nbPages,Read));
}
function ShowBook( element , i )
{
    ...
    nbPages.innerHTML = element.nbPages + " Pages";
    let group = document.createElement("div");
    let status = document.createElement("button");
    status.innerHTML = ( element.read ? "Read" : "Not Read" ) ; 
    if ( element.read ) status.classList.add("bookButtonRead");
    ...
    status.addEventListener("click",() =>
    {
        ...
        if ( status.classList.contains("bookButtonNotRead")) status.innerHTML = "Not Read" ; 
...
{
    Books.innerHTML = "";
    for ( let i = 0 ; i < myLibrary.length ; ++i ) ShowBook(myLibrary[i],i);
}
function Toggle()
{
...
submit.addEventListener("click",(e) => 
{
    ...
    let n = document.querySelector("#pages").value;
    let r = document.querySelector("#read").checked ; 
    AddBook(t,a,n,r);
